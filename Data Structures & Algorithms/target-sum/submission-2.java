class Solution {
    ...
    private static int dfs(int[] nums, int i, int target, HashMap<State,Integer> cache){
        ...
        State key = new State(i,target); 
        if(cache.containsKey(key)) return cache.get(key); 
        int p = dfs(nums,i+1,target-nums[i],cache);
        int n = dfs(nums,i+1,target+nums[i],cache); 
        cache.put(new State(i+1,target-nums[i]),p);
        cache.put(new State(i+1,target+nums[i]),n);
